...
do
  ...
  do
    ...
    if [ $Protocol -eq 0 ]; then
      pipenv run python3 execute.py "https://$WebHost:$WebPort/Shamir/128-bit-$ArraySize/shamir-party.html?arguments=-N,$NPlayers,-w,-ss,$SSHost:$SSPort,$i,innerprod" > $file 2>&1 &
    elif [ $Protocol -eq 1 ]; then
      pipenv run python3 execute.py "https://$WebHost:$WebPort/Rep-Ring/128-bit-$ArraySize/replicated-ring-party.html?arguments=-w,-ss,$SSHost:$SSPort,$i,innerprod" > $file 2>&1 &
    elif [ $Protocol -eq 2 ]; then
      ## for rep4-ring
      pipenv run python3 execute.py "https://$WebHost:$WebPort/Rep4-Ring/128-bit-$ArraySize/rep4-ring-party.html?arguments=-w,-ss,$SSHost:$SSPort,$i,innerprod" > $file 2>&1 &
    else
